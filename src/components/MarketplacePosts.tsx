...
interface MarketplacePostsProps {
  marketplaceId: string;
}

const MarketplacePosts: React.FC<MarketplacePostsProps> = ({ marketplaceId }) => {
  ...
  const fetchMarketplacePosts = async () => {
    setLoading(true);
    try {
      // Fetch posts for the marketplace
      ...
      // Fetch polls for the marketplace
      ...
      const formattedPolls = await Promise.all((pollsData || []).map(async (poll) => {
        ...
        return {
          ...
          marketplaceId: poll.marketplace_id,
        };
      }));
  ...
  if (combinedContent.length === 0) {
    return (
      <div className="text-center py-10 bg-white rounded-xl p-8 shadow-sm">
        ...
        <p className="text-muted-foreground">Be the first to post in this marketplace!</p>
      </div>
    );
  }
    ...
    <div className="space-y-6">
      {combinedContent.map((item) => (
        'question' in item ? (
          ...
          <PostCard key={item.id} post={item} onPostUpdate={fetchMarketplacePosts} />
        )
      ))}
    </div>
  );
